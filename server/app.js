...
//app.get('/write/:data/:data1/:data2', function (req, res)
app.get('/write/:data', function (req, res) {
  ...
  //data1 = strWriteReq.data1;
//  data2 = strWriteReq.data2;
  writedata(data, res);
});
...
function writeDataToMongo(_savedata, res){
  return new Promise(function(resolve, reject){
    ...
    mywritecollection.insert({
      data: Number(_savedata),    /// data: parse(_savedata).toFixed(2), 23.33989988 == 23.33
      recordTime: new Date().toLocaleString("th-TH", { timeZone: "Asia/Bangkok", hour12: false })  //new Date().getTime()
    }, function(err){
      if(err){
        console.log(err);
        res.send(String(err));
